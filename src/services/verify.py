...
class VerifyService(IVerifyService):
    ...
    async def create_email_code(self, user_email: EmailStr) -> str | None:
        async with self._uow as uow:
            ...
            redis_data = {"email": user_email, "code": verification_code, "status": "pending"}

            await uow.verify_repository.set_value(
                key=verify_token, value=json.dumps(redis_data), exp=5
            )
    ...
    async def verify_email_code(self, verify_data: VerifyCodeDTO):
        async with self._uow as uow:
            if await uow.user_repository.get_by_email(user_email=verify_data.email):
                raise UserEmailAlreadyExists()

            raw_redis_data = await uow.verify_repository.get_value(verify_data.verify_token)
            if not raw_redis_data:
                raise CodeHasExpired()

            redis_data = json.loads(raw_redis_data)
            if not (
                redis_data["email"] == verify_data.email and redis_data["code"] == verify_data.code
            ):
