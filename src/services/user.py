...
class UserService(IUserService):
    ...
    async def create(self, user_data: UserCreateDTO) -> User:
        async with self._uow as uow:
            if await uow.user_repository.get_by_email(user_email=user_data.email):
                raise UserEmailAlreadyExists()

            redis_data = await uow.verify_repository.get_value(user_data.verify_token)

            if not redis_data or json.loads(redis_data)["status"] != "verified":
                raise VerifyCodeNotConfirmed()

            await uow.verify_repository.delete_value(user_data.verify_token)

            if await uow.user_repository.get_by_username(username=user_data.username):
                raise UsernameAlreadyExists()

            hashed_password = password_hasher.hash(user_data.password).encode("utf-8")
            user_dict = user_data.model_dump(exclude={"password"})
            user_data = UserCreateDatabaseDTO(password_hash=hashed_password, **user_dict)
